...
def filter_bsdb(
    ...
    filter_method: Literal["Interactive", "Automated", "Spatial"],
    filter_field: Optional[config.FilterField] = None,
...
# automated, filter_field, filter_value, optional start_date, optional end_date)
...
def spatial_filter(
    ...
    int
        The number of rows in the filtered GeoDataFrame.
    ...
    - Raises error if input file or required columns are missing.
    """
    ...
    if not isinstance(df, gpd.GeoDataFrame) or df.geometry.name != geometry_col:
    ...
    try:
        if isinstance(polygon_input, gpd.GeoDataFrame):
            polygon_gdf = polygon_input
        else:
            polygon_gdf = gpd.read_file(polygon_input, engine="pyogrio")

    except Exception as e:
        print(f"Error loading polygon input: {e}")
        polygon_gdf = None  # or handle as appropriate

    if polygon_gdf.crs != "EPSG:3310":
        print(f"Reprojecting polygon from {polygon_gdf.crs} to EPSG:3310.")
        polygon_gdf = polygon_gdf.to_crs("EPSG:3310")

    geom_col = polygon_gdf.geometry.name
    new_columns = {
        col: f"aoi_{col}" for col in polygon_gdf.columns if col != geom_col
    }
    polygon_gdf = polygon_gdf.rename(columns=new_columns)
    
    if "aoi_index" not in polygon_gdf.columns:
        polygon_gdf["aoi_index"] = range(len(polygon_gdf))

    gdf_filtered = gpd.sjoin(gdf, polygon_gdf, predicate = "within", how = "inner")
