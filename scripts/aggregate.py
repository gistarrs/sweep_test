...
def aggregated_report(emissions_gdf, aggregates = None):
    """
    ...
    aggregates : list of str, optional
        A list of high-level grouping keys for the report. Valid values include:
            - 'YEAR', 'MONTH', 'INCIDENT', 'COABDIS', 'COUNTY',
              'DISTRICT', 'DISTRICT ID', 'AIR BASIN'
        If None or empty, defaults to ['YEAR', 'INCIDENT'].
    ...
    aggregate_cols = {
        ...
        "COABDIS" : ['COABDIS', 5],
        "COUNTY": ['COUNTY', 4], 
        "DISTRICT": ['AIR_DISTRICT', 3], 
        "DISTRICT ID": ['DISA_ID', 3], 
    ...
    # if "FID_AOI_TA" in structure_df.columns:
    #     structure_df.rename(columns={"FID_AOI_TA": "POLYGON_ID"}, inplace=True)
    #     sorted_columns.append("POLYGON_ID")
